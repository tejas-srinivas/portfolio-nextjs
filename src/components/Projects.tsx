...
import { projects, projectSectionContent, Project } from "@/lib/data";
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="font-mono text-base font-bold leading-snug md:text-lg" style={{ color: "#e6edf3" }}>
            {project.name}
          </h3>
        </div>
        {featured && (
          <span
            className="shrink-0 rounded-full px-3 py-1 font-mono text-[10px] tracking-widest"
            style={{ background: "#0d1117", color: "#3fb950", border: "1px solid #30363d" }}
          >
            {projectSectionContent.featuredBadgeLabel}
          </span>
        )}
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        >
          {projectSectionContent.scopeLabel}: {project.scope}
        </span>
      </div>

      {/* Description */}
      <p className="mt-4 text-sm leading-relaxed" style={{ color: "#8b949e" }}>
        {project.description}
      </p>
      ...
      >
        <p className="font-mono text-[10px] tracking-widest" style={{ color: "#3fb950" }}>
          {projectSectionContent.impactLabel}
        </p>
        <p className="mt-2 font-mono text-sm md:text-base" style={{ color: "#e6edf3" }}>
          {project.metric}
        </p>
      </div>

      <div className="mt-5">
        <p className="font-mono text-[10px] tracking-widest mb-3" style={{ color: "#58a6ff" }}>
          {projectSectionContent.highlightsLabel}
        </p>
        <ul className="space-y-2">
          {project.highlights.map((highlight) => (
            <li key={highlight} className="flex gap-2 text-sm leading-relaxed" style={{ color: "#8b949e" }}>
              <span className="font-mono text-xs mt-1" style={{ color: "#3fb950" }}>
                →
              </span>
              <span>{highlight}</span>
            </li>
      ...
      {/* Tech badges */}
      <div className="mt-6">
        <p className="font-mono text-[10px] tracking-widest mb-3" style={{ color: "#8b949e" }}>
          {projectSectionContent.stackLabel}
        </p>
        <div className="flex flex-wrap gap-2">
          {project.tech.map((tech) => (
            <span
              ...
              style={{ background: "#21262d", color: getTechColor(tech), border: "1px solid #30363d" }}
            >
              {tech}
            </span>
          ))}
      ...
      <div className="mt-6 flex flex-wrap items-center gap-4">
        ...
        {project.github && (
          <a
            ...
            className="inline-flex items-center gap-2 rounded px-4 py-2 font-mono text-xs transition-opacity duration-200 hover:opacity-80"
          ...
          >
            ...
            {projectSectionContent.githubLabel}
          </a>
        )}
        ...
        >
          {projectSectionContent.secondaryCtaLabel} →
        </a>
      </div>
    </motion.article>
  );
  ...
  return (
    ...
    >
      ...
      <motion.p
        ...
        style={{ color: "#3fb950" }}
      >
        {projectSectionContent.terminalLabel}
      </motion.p>
      ...
      <motion.h2
        ...
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, margin: "-100px" }}
        transition={{ duration: 0.5, ease: "easeOut", delay: 0.08 } as Transition}
        className="font-mono text-2xl font-bold leading-tight md:text-3xl"
        style={{ color: "#e6edf3" }}
      >
        {projectSectionContent.heading}
      </motion.h2>
      ...
      <motion.p
        ...
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, margin: "-100px" }}
        transition={{ duration: 0.5, ease: "easeOut", delay: 0.14 } as Transition}
        className="mt-4 mb-10 max-w-3xl text-sm leading-relaxed md:text-base"
        style={{ color: "#8b949e" }}
      >
        {projectSectionContent.description}
      </motion.p>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <motion.div
          ...
          className="rounded-lg p-6 flex items-center justify-center md:col-span-2"
          style={{ background: "transparent", border: "1px dashed #30363d" }}
        >
          <p className="font-mono text-sm" style={{ color: "#30363d" }}>
            {projectSectionContent.moreComingLabel}
          </p>
        </motion.div>
      </div>
    </section>
